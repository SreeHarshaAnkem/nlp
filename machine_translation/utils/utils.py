...
class ModelJob:
        ...
        for epoch in range(1, self.n_epochs + 1):
            ...
            for mode in self.phases:
                ...
                for n_batches, batch in enumerate(self.dataloaders[mode]):
                    ...
                    # forward propogation
                    y_pred, state = self.model(batch_X)
                    # compute loss
    ...
    def predict_step(self, dataloader=None, h_t_1=None, if_y=False):
        self.model.eval()
        for n_batches, batch in enumerate(dataloader):
            if if_y:
                ...
                batch_X = batch
            # forward propogation
            y_pred, state = self.model(batch_X, h_t_1)
            if n_batches == 0:
                predictions = y_pred
                if if_y:
                    all_y = batch_y
        ...
        if if_y:
            return all_y, predictions,state
        else:
...
class Vocab:
    def __init__(self, df, min_frequency, vocab_size, field, include_unk):
        self.df = df
        ...
        if self.include_unk:
            self.vocab["<UNK>"] = self.vocab_size-1
            self.vocab_ctoi[self.vocab_size-1] = "<UNK>"
    ...
    def build_vocab(self):
        ...
        freq_dict = freq_dict[:self.vocab_size - (2 if self.include_unk==True else 1)]
        freq_dict = dict(freq_dict)
        vocab = {token : idx+1 for idx, (token, _) in enumerate(freq_dict.items())}
        vocab_ctoi = {idx+1:token for idx, (token, _) in enumerate(freq_dict.items())}
